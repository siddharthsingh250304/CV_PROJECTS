...
def encode_oriented_boxes(reference_boxes: Tensor, proposals: Tensor, weights: Tensor) -> Tensor:
    wx = weights[0]
    wy = weights[1]
    ww = weights[2]
    wh = weights[3]
    ...
    reference_boxes_x2 = reference_boxes[:, 2].unsqueeze(1)
    reference_boxes_y2 = reference_boxes[:, 3].unsqueeze(1)
    reference_boxes_theta = reference_boxes[:, 4].unsqueeze(1)
    
    ex_widths = proposals_x2 - proposals_x1
    ex_heights = proposals_y2 - proposals_y1
    ex_ctr_x = proposals_x1 + 0.5 * ex_widths
    ex_ctr_y = proposals_y1 + 0.5 * ex_heights
    ex_theta = proposals_theta
    ...
    targets_dw = ww * torch.log(gt_widths / ex_widths)
    targets_dh = wh * torch.log(gt_heights / ex_heights)
    targets_dtheta = wtheta * (reference_boxes_theta - ex_theta)
    
    targets = torch.cat((targets_dx, targets_dy, targets_dw, targets_dh, targets_dtheta), dim=1)
    return targets
...
class OrientedBoxCoder:
    def __init__(self, weights: Tuple[float, float, float, float, float], bbox_xform_clip: float = math.log(1000.0 / 16)) -> None:
        print("OrientedBoxCoder")
        print(weights)
        self.weights = weights
        self.bbox_xform_clip = bbox_xform_clip
    ...
    def encode_single(self, reference_boxes: Tensor, proposals: Tensor) -> Tensor:
        ...
        device = reference_boxes.device
        weights = torch.as_tensor(self.weights, dtype=dtype, device=device)
        targets = encode_oriented_boxes(reference_boxes, proposals, weights)
        return targets
    ...
    def decode(self, rel_codes: Tensor, boxes: List[Tensor]) -> Tensor:
        ...
        if box_sum > 0:
            rel_codes = rel_codes.reshape(box_sum, -1)
        pred_boxes, pred_thetas = self.decode_single(rel_codes, concat_boxes)
        if box_sum > 0:
            pred_boxes = pred_boxes.reshape(box_sum, -1, 4)
            pred_thetas = pred_thetas.reshape(box_sum, -1)
        
        
        return pred_boxes, pred_thetas
    
    def decode_single(self, rel_codes: Tensor, boxes: Tensor) -> Tensor:
    
        """
        Decode a set of relative box offsets into oriented bounding boxes.

        Args:
            rel_codes (Tensor): encoded offsets for each box.
            boxes (Tensor): reference boxes with format (x1, y1, x2, y2).

        Returns:
            Tensor: decoded rotated boxes with format (cx, cy, w, h, theta).
        """
        boxes = boxes.to(dtype=rel_codes.dtype)

        # Extract reference box parameters
        widths = boxes[:, 2] - boxes[:, 0]
        heights = boxes[:, 3] - boxes[:, 1]
        ctr_x = boxes[:, 0] + 0.5 * widths
        ctr_y = boxes[:, 1] + 0.5 * heights
        ...
        # Extract encoded offsets
        wx, wy, ww, wh, wtheta = self.weights
        dx = rel_codes[:, 0] / wx
        dy = rel_codes[:, 1] / wy
        dw = rel_codes[:, 2] / ww
        dh = rel_codes[:, 3] / wh
        dtheta = rel_codes[:, 4] / wtheta
        ...
        pred_boxes3 = pred_ctr_x + c_to_c_w
        pred_boxes4 = pred_ctr_y + c_to_c_h
        #also return base boxes
        pred_boxes = torch.stack((pred_boxes1, pred_boxes2, pred_boxes3, pred_boxes4), dim=2).flatten(1)
        return pred_boxes, pred_theta

    def decode_unchange(self, rel_codes: Tensor, boxes: List[Tensor]) -> Tensor:
        torch._assert(
            isinstance(boxes, (list, tuple)),
            "This function expects boxes of type list or tuple.",
        )
        ...
        if box_sum > 0:
            rel_codes = rel_codes.reshape(box_sum, -1)
        pred_boxes, pred_thetas = self.decode_single_unchange(rel_codes, concat_boxes)
        if box_sum > 0:
            pred_boxes = pred_boxes.reshape(box_sum, -1, 4)
            pred_thetas = pred_thetas.reshape(box_sum, -1)
    ...
    def decode_single_unchange(self, rel_codes: Tensor, boxes: Tensor) -> Tensor:
