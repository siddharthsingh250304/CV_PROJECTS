...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def postprocess(
        ...
        for i, (pred, im_s, o_im_s) in enumerate(zip(result, image_shapes, original_image_sizes)):
            ...
            boxes_un = pred["boxes_un"]
            boxes_un = resize_boxes(boxes_un, im_s, o_im_s)
            result[i]["boxes"] = boxes
            result[i]["boxes_un"] = boxes_un
            if "thetas_un" in pred:
                thetas_un = pred["thetas_un"]
                result[i]["thetas_un"] = thetas_un
            if "thetas" in pred:
                thetas = pred["thetas"]
                result[i]["thetas"] = thetas  # No change unless aspect ratio transforms are applied
            if "masks" in pred:
                masks = pred["masks"]
